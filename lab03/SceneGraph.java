...
public class SceneGraph extends JPanel {
	...
	private TransformedObject threadmill_wheel_1;
	private TransformedObject threadmill_wheel_2;
...
////		 TODO: Create objects and add them to the scene graph.
		...
		threadmill_wheel_1 = new TransformedObject(polygonPath);
		threadmill_wheel_2 = new TransformedObject(polygonPath);
		
		var threadmill_bar = new CompoundObject();
		
		threadmill_bar.add(
				threadmill_wheel_1.setTranslation(-1.25, 2)
					.setColor(Color.BLACK)
		);
		
		threadmill_bar.add(
				threadmill_wheel_2.setTranslation(1.25, 1.5)
					.setColor(Color.BLACK)
		);
		
		threadmill_bar.add(
				new TransformedObject(filledRect).setScale(2.65, 0.18)
					.setRotation(-10).setTranslation(0, 1.75)
					.setColor(Color.RED)
		);
		
		var threadmill_object = new CompoundObject();
		
		threadmill_object.add(
				threadmill_bar
			);
		
		threadmill_object.add(
				new TransformedObject(filledTriangle).setScale(0.5, 1.75)
				);
		
		var threadmill_1 = new TransformedObject(threadmill_object)
			.setTranslation(0, -1.75).setColor(Color.BLUE);
		
		var threadmill_2 = new TransformedObject(threadmill_object)
			.setScale(0.5, 0.5).setTranslation(2, 0.5).setColor(darkGreen);
		
		var threadmill_3 = new TransformedObject(threadmill_object)
				.setScale(0.75, 0.75).setTranslation(-2, 0.5).setColor(purple);
		
		world.add(threadmill_1);
		world.add(threadmill_2);
		world.add(threadmill_3);
	...
	public void updateFrame() {
		...
		threadmill_wheel_1.setRotation(frameNumber*0.75);
		threadmill_wheel_2.setRotation(frameNumber*0.75);
	...
	private static Path2D CreatePolygonPath(int numberOfSides, int radius) {
	...
	private static SceneGraphNode polygonPath = new SceneGraphNode() {
...
//				drawGuideLines(g2); // to disable after finish
				world.draw(g2);
			}
		};
