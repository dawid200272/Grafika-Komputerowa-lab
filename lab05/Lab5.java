...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		case 1:
			drawCorkscrew(gl2, 1, 18);
			break;
		case 2:
			drawPyramid(gl2, 5, 18);
			break;
	...
	private void drawCorkscrew(GL2 gl2,double size, int numberOfRotations) {
		
		gl2.glPushMatrix();
		
		gl2.glEnable(GL2.GL_POINT_SPRITE);
		gl2.glEnable(GL2.GL_PROGRAM_POINT_SIZE_EXT);
...
//		gl2.glEnd();
	...
	private void drawPyramid(GL2 gl2, double size, int numberOfSides) {
		...
		gl2.glScaled(size, size, size);
		gl2.glRotated(-45, 1, 0, 0); // (-90, 1, 0, 0)
	...
	private void drawTriangle(GL2 gl2, double f, double s, int numberOfSides, double z) {		
		...
		gl2.glVertex3d(Math.cos(f * 2 * Math.PI / numberOfSides), Math.sin(f * 2 * Math.PI / numberOfSides), 0);
		gl2.glVertex3d(Math.cos(s * 2 * Math.PI / numberOfSides), Math.sin(s * 2 * Math.PI / numberOfSides), 0);
		gl2.glVertex3d(0, 0, z);
